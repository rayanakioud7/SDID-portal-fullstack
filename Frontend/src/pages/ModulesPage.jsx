...
const ModulesPage = () => {
  ...
  const [filteredModules, setFilteredModules] = useState([]); // 👈 NEW: For filtering
  const [instructors, setInstructors] = useState([]); // 👈 NEW: List of unique professors
  const [selectedInstructor, setSelectedInstructor] = useState("All Professors"); // 👈 NEW: Active filter
  ...
  const fetchStudentCourses = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFilteredModules(formattedModules); // Initially show all
